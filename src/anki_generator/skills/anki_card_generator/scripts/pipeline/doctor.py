...
from anki_generator.skills.anki_card_generator.scripts.schemas import CmdDoctorResponse
from . import core
...
def cmd_doctor(db_path=None) -> tuple[CmdDoctorResponse, int]:
    ...
    try:
        probe = core.MEDIA_DIR / ".doctor_probe"
        probe.write_text("ok")
        probe.unlink()
        add("media_dir", True, str(core.MEDIA_DIR))
    except Exception as e:
        add("media_dir", False, str(e))

    try:
        link = core.PROJECT_ROOT / ".agents" / "skills" / "anki_card_generator"
        if link.exists() and link.resolve() == core.SKILL_DIR:
    ...
    # Dynamically read config for ANKI_ENABLED
    ...
    try:
        ...
        if core.ANKI_NOTE_MODEL in models:
            add("anki_connect", True, f"note model '{core.ANKI_NOTE_MODEL}' present")
        else:
            add("anki_connect", True,
                f"note model '{core.ANKI_NOTE_MODEL}' missing — will be created on first push")
    except Exception as e:
    ...
    try:
        ...
        if tracked:
            in_anki = set(anki_connector.invoke(
                "findNotes", query=f'"note:{core.ANKI_NOTE_MODEL}"'))
