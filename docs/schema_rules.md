...
4. **Sentence translation length ratio**: `back_meaning` must translate the full `front` example sentence, not just the target word. If `front` is a sentence (len >= 15 chars) and `back_meaning` length is suspiciously short (ratio < 0.30 vs `front` or < 6 chars), the validator flags a hard failure. **Text in parentheses does not count** toward that length: `(의문·불신·야망 등 감정이나 생각을) *품다*, 갖다` is a word gloss, not a sentence translation, and padding the gloss will not satisfy this rule. A gloss *plus* the sentence translation is the normal, accepted shape.
5. **Reading cross-validation** is a **warning, never a failure** — for `root_id` and for every bracketed word in `back_reading`. A warning means the reading is not one the dictionary lists for that surface, so it is worth a second look for a typo; **do not regenerate the card over it**. Alternative readings (間[あいだ] vs 間[ま]), rendaku, counters, and words the dictionary does not know are deliberately silent, so a warning that does appear is worth reading.
